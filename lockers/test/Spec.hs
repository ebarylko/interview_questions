...
import Lib (createNLockers, Lockers(..), LockerSize(..),  size, toPositive, toLockerIds, toLocker,  removePackage, LockerRemovalError(..))
...
main = hspec $ do
  context "Lockers" $ do
    ...
    -- describe "When removing a package from a locker that does not exist" $ do
    --   it "An invalid locker id error is returned" $ do
    --     let invalidId = toLockerId 2
